...
It covers the active SDL app, legacy C++ CLI tooling, and `phigros_cpp` evaluation workflows. It does not document the historical standalone Python renderer path.

## What Uses Config

- `phigros_sdl_app`: runtime renderer/player config model used by in-app defaults and toggles
- `phigros_render`: legacy CLI config loaded from `--config` when `BUILD_LEGACY_CLI=ON`
...
See [CPP_RENDERER.md](CPP_RENDERER.md) for app and legacy CLI usage, and [../cpp/docs/CONFIG.md](../cpp/docs/CONFIG.md) for the internal field reference.
...
- Want to know how config relates to the SDL app and legacy CLI: [CPP_RENDERER.md](CPP_RENDERER.md)
