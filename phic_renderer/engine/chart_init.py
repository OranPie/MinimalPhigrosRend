...
def compute_total_notes(
    ...
    In composite mode with duplicate tracks, deduplicates based on source chart path
    to show the correct MAX COMBO (counts each unique note once, not per track).
    """
    if advance_active and advance_cfg and advance_cfg.get("mode") == "composite":
        ...
        for track in tracks:
            ...
            try:
                chart_p_norm = os.path.normpath(os.path.abspath(str(chart_p)))
            except Exception:
                chart_p_norm = str(chart_p)

            _fmt_i, _off_i, _lines_i, notes_i = load_chart(chart_p, W, H)
            for n in notes_i:
                if not n.fake:
                    # Include chart path in deduplication key to handle same source used multiple times
                    unique_notes.add((chart_p_norm, n.nid, n.t_hit, n.line_id))
